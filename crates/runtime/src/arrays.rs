...
pub(super) fn get_f32(env: &HashMap<String, Value>, array: &str, index: i64) -> Result<F32Value, AppError> {
    let index = usize::try_from(index).map_err(|_| AppError::BadRequest)?;
    let Some(Value::F32Array(items)) = env.get(array) else { return Err(AppError::Internal); };
