...
use data::Database;
use language_core::{AppError, Program, TxStatement, Value, TRANSACTION_OUTCOME_ENUM_ID};
...
pub(crate) async fn execute(
    ...
    for statement in statements {
        let result = match statement {
            TxStatement::LetQuery { name, call } => execute_tx_query(program, call, env, budget, &mut tx)
                .await
                .and_then(|value| {
            ...
            TxStatement::Query(call) => execute_tx_query(program, call, env, budget, &mut tx).await.map(|_| ()),
            TxStatement::BusinessAudit(audit) => write_business_audit(audit, env, budget, &mut tx).await,
        };
    ...
    if let Some(error) = failed {
        ...
        let variant = if tx.rollback().await.is_ok() { "RolledBack" } else { "CommitUnknown" };
        set_outcome(env, outcome.as_ref().expect("checked"), variant);
        return Ok(());
    }
    if let Some(name) = outcome {
        let variant = if tx.commit().await.is_ok() { "Committed" } else { "CommitUnknown" };
...
fn set_outcome(env: &mut HashMap<String, Value>, name: &str, variant: &str) {
    env.insert(
        name.to_string(),
        Value::Enum { enum_id: TRANSACTION_OUTCOME_ENUM_ID, variant: variant.to_string() },
    );
}
