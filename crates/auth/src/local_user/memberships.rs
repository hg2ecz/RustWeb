...
impl LocalUserStore {
    pub async fn memberships(&self, username: &str) -> Result<Vec<TenantId>, AuthError> {
        ...
        let rows = self
            ...
            .fetch_all(
                &PreparedSql::compile(
                    "SELECT tenant_id FROM _rw_local_user_memberships WHERE username=:username ORDER BY tenant_id",
                )
                .map_err(|_| AuthError::Internal)?,
                &b,
                &shape,
    ...
    pub async fn set_memberships(
        ...
        tx.execute(
            &PreparedSql::compile(
                "DELETE FROM _rw_local_user_memberships WHERE username=:username",
            )
            .map_err(|_| AuthError::Internal)?,
            &b,
        )
        ...
        for tenant in memberships {
            ...
            tx.execute(
                &PreparedSql::compile(
                    "INSERT INTO _rw_local_user_memberships(username,tenant_id) VALUES(:username,:tenant)",
                )
                .map_err(|_| AuthError::Internal)?,
                &mb,
            )
        ...
        tx.execute(
            &PreparedSql::compile(
                "UPDATE _rw_local_users SET auth_generation=auth_generation+1 WHERE username=:username",
            )
            .map_err(|_| AuthError::Internal)?,
            &b,
        )
