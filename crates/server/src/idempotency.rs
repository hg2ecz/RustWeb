...
enum StoredState {
    ...
    Complete { fingerprint: String, response: StoredResponse },
}
...
pub(super) async fn begin(
    ...
    let Some(principal) = principal else {
        return Err(error(json_api, 401, "Unauthorized", "authentication_required", b"authentication required\n"));
    };
    let Some(raw) = request.header("idempotency-key") else {
        return Err(error(json_api, 400, "Bad Request", "idempotency_key_required", b"Idempotency-Key header required\n"));
    };
    if !valid_key(raw) {
        return Err(error(json_api, 400, "Bad Request", "invalid_idempotency_key", b"invalid Idempotency-Key\n"));
    }
    let Some(redis) = redis else {
        return Err(error(json_api, 503, "Service Unavailable", "idempotency_unavailable", b"idempotency unavailable\n"));
    };
    let storage_key = storage_key(domain_namespace, &route.name, principal, raw);
    let fingerprint = request_fingerprint(request);
    let pending = StoredState::Pending { fingerprint: fingerprint.clone() };
    let encoded = serde_json::to_vec(&pending).map_err(|_| unavailable(json_api))?;
    match redis.set_if_absent(&storage_key, &encoded, IDEMPOTENCY_TTL_SECS).await {
        Ok(true) => Ok(BeginResult::Claimed(IdempotencyClaim { storage_key, fingerprint })),
...
pub(super) async fn complete(
    ...
    let Ok(encoded) = serde_json::to_vec(&state) else { return; };
    let _ = redis.set(&claim.storage_key, &encoded, Some(IDEMPOTENCY_TTL_SECS)).await;
}
...
async fn load_existing(
    ...
    let bytes = redis.get(key).await.map_err(|_| unavailable(json_api))?
        .ok_or_else(|| unavailable(json_api))?;
    let state: StoredState = serde_json::from_slice(&bytes).map_err(|_| unavailable(json_api))?;
    match state {
        StoredState::Pending { fingerprint: stored } => {
            if stored != fingerprint {
                Err(key_reused(json_api))
            } else {
                Err(error(json_api, 409, "Conflict", "request_in_progress", b"idempotent request is still in progress\n"))
            }
        }
        StoredState::Complete { fingerprint: stored, response } => {
            if stored != fingerprint {
                Err(key_reused(json_api))
            } else {
                response.into_response().map(BeginResult::Replay).map_err(|_| unavailable(json_api))
            }
        }
    }
}
...
fn key_reused(json_api: bool) -> Response {
    error(json_api, 409, "Conflict", "idempotency_key_reused", b"Idempotency-Key was already used for another request\n")
}

fn unavailable(json_api: bool) -> Response {
    error(json_api, 503, "Service Unavailable", "idempotency_unavailable", b"idempotency unavailable\n")
}

fn error(json_api: bool, status: u16, reason: &'static str, code: &str, text: &[u8]) -> Response {
    if json_api {
        Response::new(status, reason, "application/json; charset=utf-8", format!("{{\"error\":\"{code}\"}}\n").as_bytes())
    } else {
...
mod tests {
    ...
    fn fingerprint_changes_with_payload() {
        ...
        assert_ne!(request_fingerprint(&request(b"a")), request_fingerprint(&request(b"b")));
    }
}
