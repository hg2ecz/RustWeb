...
pub(super) fn parse_match(
    ...
    let open = body[expr_start..]
        ...
        .ok_or_else(|| CompileError::Syntax(format!("{handler_kind} `{handler_name}` match missing {{")))?;
    let expr_text = body[expr_start..open].trim();
    if expr_text.is_empty() {
        return Err(CompileError::Syntax(format!("{handler_kind} `{handler_name}` match expression expected")));
    }
    ...
    let (sum_name, variants): (String, Vec<String>) = if enum_id == language_core::TRANSACTION_OUTCOME_ENUM_ID {
        ("TransactionOutcome".into(), language_core::TRANSACTION_OUTCOME_VARIANTS.iter().map(|value| (*value).to_string()).collect())
    } else {
        let definition = program.enum_by_id(enum_id).ok_or_else(|| CompileError::Syntax("match references unknown sum type".into()))?;
        (definition.name.clone(), definition.variants.clone())
    };
    let close = matching_brace(body, open)
        .ok_or_else(|| CompileError::Syntax(format!("{handler_kind} `{handler_name}` match block unclosed")))?;
    ...
    while arm_cursor < close {
        ...
        let variant = read_ident(body, arm_cursor).ok_or_else(|| {
            CompileError::Syntax(format!("{handler_kind} `{handler_name}` match variant expected"))
        })?;
        ...
        if arms.iter().any(|arm: &ParsedMatchArm| arm.variant == variant) {
            return Err(CompileError::security(
                "SEC-A10-022",
        ...
        if !body[next..close].starts_with("=>") {
            return Err(CompileError::Syntax(format!("match arm `{variant}` requires `=>`")));
        }
        next = skip_ws_and_comments(body, next + 2);
        if body.as_bytes().get(next) != Some(&b'{') {
            return Err(CompileError::Syntax(format!("match arm `{variant}` requires a block")));
        }
        let arm_close = matching_brace(body, next)
            .ok_or_else(|| CompileError::Syntax(format!("match arm `{variant}` block unclosed")))?;
        if arm_close > close {
            return Err(CompileError::Syntax(format!("match arm `{variant}` escapes enclosing match")));
        }
    ...
    Ok(ParsedMatch { expr, enum_id, arms, close })
}
