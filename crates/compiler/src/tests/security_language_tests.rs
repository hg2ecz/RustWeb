...
page fn account(ctx: PageContext, id: Int) -> Result<Html, PageError> { return Ok(html {account}); }
route account GET "/account/:id<Int>" public => account;
route go POST "/go" public => go;
"#;
...
query fn loadAccount(db: Db, id: Int) -> Result<Account, DbError> sql {
    SELECT id, passwordHash FROM accounts WHERE id = :id
}
page fn account(ctx: PageContext, db: Db, id: Int) -> Result<Json, PageError> {
    let account = loadAccount(db, id)?;
    let leaked = account.passwordHash;
    return Ok(json(leaked));
}
route account GET "/accounts/:id<Int>" auth user => account;
"#;
...
page fn account(ctx: PageContext, db: Db, id: Int) -> Result<Html, PageError> {
    let account = loadAccount(db, id)?;
...
query fn loadProfile(db: Db, id: Int) -> Result<Profile, DbError> sql {
    SELECT id, owner, email FROM profiles WHERE id = :id
}
page fn profile(ctx: PageContext, db: Db, id: Int) -> Result<Json, PageError> {
...
page fn profile(ctx: PageContext, db: Db, id: Int) -> Result<Html, PageError> {
    let profile = loadProfile(db, id)?;
    authorize profile owner owner;
...
action fn edit(ctx: ActionContext, db: Db, id: Int, title: String) -> Result<Json, PageError> {
    transaction db {
...
query fn loadComment(db: Db, id: Int) -> Result<Comment, DbError> sql {
...
fn split_bounded_requires_compile_time_item_limit() {
    let src = r#"
page fn tags(ctx: PageContext, raw: String, max: Int) -> Result<Json, PageError> {
    let items = splitBounded(raw, ",", max);
    return Ok(json(len(items)));
}
route tags GET "/tags" query raw<String> max<Int> public => tags;
"#;
...
fn nominal_domain_ids_are_not_interchangeable() {
    let src = r#"
type UserId = Int { range 1 999999; }
type ArticleId = Int { range 1 999999; }
...
"#;
    let err = compile_source(src).expect_err("Int literal must not silently become UserId");
    let msg = err.to_string();
    assert!(msg.contains("SEC-TYPE-001"), "{msg}");
    assert!(msg.contains("UserId"), "{msg}");
}
...
page fn show(ctx: PageContext, db: Db, raw: Int) -> Result<Json, PageError> {
    let userId = validate UserId(raw)?;
    let user = loadUser(db, userId)?;
    return Ok(json(true));
}
route show GET "/users" query raw<Int> public => show;
"#;
...
"#;
    let err = compile_source(src).expect_err("String must not be refinable into Int-backed UserId");
    let msg = err.to_string();
...
fn domain_validation_requires_explicit_failure_propagation() {
    let src = r#"
type UserId = Int { range 1 999999; }
page fn show(ctx: PageContext, raw: Int) -> Result<Json, PageError> {
    let userId = validate UserId(raw);
    return Ok(json(true));
}
route show GET "/users" query raw<Int> public => show;
...
query fn loadUser(db: Db, id: Int) -> Result<User, DbError> sql {
...
route show GET "/users/:id<Int>" auth user => show;
"#;
...
query fn loadCredential(db: Db, id: Int) -> Result<Credential, DbError> sql {
    SELECT id, token FROM credentials WHERE id = :id
}
query fn lookupByToken(db: Db, token: Secret<String>) -> Result<Credential?, DbError> sql {
    SELECT id, token FROM credentials WHERE token = :token
}
page fn check(ctx: PageContext, db: Db, id: Int) -> Result<Json, PageError> {
    let credential = loadCredential(db, id)?;
    let found = lookupByToken(db, credential.token)?;
    return Ok(json(true));
}
route check GET "/credentials/:id<Int>" auth user => check;
"#;
...
query fn unsafeLookup(db: Db, token: String) -> Result<Credential?, DbError> sql {
    SELECT id, token FROM credentials WHERE token = :token
}
page fn check(ctx: PageContext, db: Db, id: Int) -> Result<Json, PageError> {
    let credential = loadCredential(db, id)?;
...
query fn touch(tx: Transaction, id: Int) -> Result<Changed, DbError> mutates Account by id sql {
    UPDATE accounts SET owner = owner WHERE id = :id
}
action fn update(ctx: ActionContext, db: Db, id: Int) -> Result<Json, PageError> {
    let account = loadAccount(db, id)?;
...
route update POST "/accounts/:id<Int>" auth user => update;
"#;
...
query fn storeHash(tx: Transaction, id: Int, hash: Secret<PasswordHash>) -> Result<Void, DbError> mutates Account by id sql {
    UPDATE accounts SET passwordHash = :hash WHERE id = :id
}
query fn loadAccount(db: Db, id: Int) -> Result<Account, DbError> sql {
    SELECT id, passwordHash FROM accounts WHERE id = :id
}
action fn change(ctx: ActionContext, db: Db, id: Int, password: Password) -> Result<Redirect, PageError> {
    let account = loadAccount(db, id)?;
...
fn password_hash_cannot_flow_into_public_sink() {
    let src = r#"
query fn unsafeStore(tx: Transaction, hash: String) -> Result<Void, DbError> sql {
...
page fn check(ctx: PageContext, db: Db, id: Int, password: Password) -> Result<Json, PageError> {
    let credential = loadCredential(db, id)?;
    let valid = passwordVerify(credential.token, password);
    return Ok(json(valid));
}
route check POST "/check/:id<Int>" form password<Password> auth user => check;
"#;
...
fn session_token_generation_flows_only_into_matching_secret_sink() {
    let src = r#"
query fn storeSession(tx: Transaction, token: Secret<SessionTokenHash>) -> Result<Void, DbError> sql {
...
query fn loadSession(db: Db, id: Int) -> Result<Session, DbError> sql {
    SELECT id, token_hash AS tokenHash, expires_at AS expiresAt FROM sessions WHERE id = :id
}
action fn check(ctx: ActionContext, db: Db, id: Int, token: SessionToken) -> Result<Json, PageError> {
    let session = loadSession(db, id)?;
    let valid = tokenActive(session.tokenHash, token, session.expiresAt);
    return Ok(json(valid));
}
route check POST "/sessions/:id<Int>/check" form token<SessionToken> auth user => check;
"#;
...
query fn loadReset(db: Db, id: Int) -> Result<Reset, DbError> sql {
    SELECT id, token_hash AS tokenHash, expires_at AS expiresAt FROM resets WHERE id = :id
}
page fn check(ctx: PageContext, db: Db, id: Int, token: SessionToken) -> Result<Json, PageError> {
...
route check POST "/check/:id<Int>" form token<SessionToken> auth user => check;
"#;
...
query fn loadState(db: Db, id: Int) -> Result<FormState, DbError> sql {
    SELECT id, token_hash AS tokenHash FROM form_state WHERE id = :id
}
action fn check(ctx: ActionContext, db: Db, id: Int, token: CsrfToken) -> Result<Json, PageError> {
...
query fn loadImported(db: Db, id: Int) -> Result<ImportedHash, DbError> sql {
    SELECT id, token_hash AS tokenHash FROM imported_hashes WHERE id = :id
}
query fn consumeReset(tx: Transaction, tokenHash: PasswordResetTokenHash) -> Result<Changed, DbError> consumes ResetGrant by tokenHash before expiresAt sql {
    DELETE FROM reset_grants WHERE token_hash = :tokenHash AND expires_at > CURRENT_TIMESTAMP
}
action fn reset(ctx: ActionContext, db: Db, id: Int) -> Result<Json, PageError> {
...
query fn consumeReset(tx: Transaction, tokenHash: PasswordResetTokenHash) -> Result<Void, DbError> consumes ResetGrant by tokenHash before expiresAt sql {
...
action fn rotate(ctx: ActionContext, db: Db, id: Int, token: SessionToken, expiresAt: DateTime) -> Result<Json, PageError> {
    let session = loadSession(db, id)?;
    let oldHash = presentedTokenHash(token);
    transaction db {
...
query fn loadImported(db: Db, id: Int) -> Result<ImportedSessionSecret, DbError> sql {
    SELECT id, token FROM imported_session_secrets WHERE id = :id
}
query fn storeHash(tx: Transaction, tokenHash: Secret<SessionTokenHash>) -> Result<Void, DbError> sql {
    INSERT INTO session_hashes(token_hash) VALUES(:tokenHash)
}
action fn persist(ctx: ActionContext, db: Db, id: Int) -> Result<Json, PageError> {
    let imported = loadImported(db, id)?;
...
action fn removeUser(ctx: ActionContext, id: Int) -> Result<Json, PageError> requires UserAdmin {
    return Ok(json(true));
}
...
route removeUser POST "/users/:id<Int>/remove" auth user => removeUser;
"#;
...
fn route_rejects_unknown_named_permission() {
    let src = r#"
action fn removeUser(ctx: ActionContext, id: Int) -> Result<Json, PageError> {
    return Ok(json(true));
}
route removeUser POST "/users/:id<Int>/remove" auth permission UserAdmin => removeUser;
"#;
...
action fn refund(ctx: ActionContext, id: Int) -> Result<Json, PageError> requires BillingWrite + mfa {
    return Ok(json(true));
}
