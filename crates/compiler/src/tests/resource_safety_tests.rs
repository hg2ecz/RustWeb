...
fn list_query_requires_literal_row_bound() {
    let source = r#"
model Item { id: Int }
query fn items(db: Db) -> Result<List<Item>, DbError> sql {
...
fn list_query_rejects_parameterized_limit() {
    let source = r#"
model Item { id: Int }
query fn items(db: Db, limit: Int) -> Result<List<Item>, DbError> sql {
    SELECT id FROM items LIMIT :limit
}
page fn index(ctx: PageContext, db: Db, limit: Int) -> Result<Json, PageError> {
    let items = items(db, limit)?;
    return Ok(json(expose(items, id)));
}
route index GET "/" query limit<Int> validate limit range 1 1000 public => index;
"#;
