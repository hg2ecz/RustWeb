...
query fn load(db: Db, id: Int) -> Result<Item, DbError> sql {
...
page fn show(ctx: PageContext, db: Db, id: Int) -> Result<Json, PageError> uses Catalog {
    let item = load(db, id)?;
    return Ok(json(expose(item, id)));
}

route show GET "/items/:id<Int>" public => show;
"#;
