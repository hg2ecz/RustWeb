...
query fn renameProduct(tx: Transaction, id: Int, name: String) -> Result<Product, DbError> mutates Product by id sql {
...
route rename POST "/products/:id<Int>/rename" form name<String> auth user => rename;
...
query fn loadCounter(db: Db, id: Int) -> Result<Counter, DbError> sql {
    SELECT id, version FROM counters WHERE id = :id
}
query fn bump(tx: Transaction, id: Int, version: Int) -> Result<Changed, DbError> mutates Counter by id sql {
...
route echo POST "/api/echo" json name<String> age<Int> active<Bool> validate name length 1 100 age range 0 150 public => echo;
"#;
        let p = compile_source(src).unwrap();
...
route echo POST "/api/echo" form name<String> json age<Int> public => echo;
"#;
...
route calc POST "/api/calc" json net<Decimal> count<Int> public => calc;
"#;
        assert!(compile_source(src).is_err());
    }
}
