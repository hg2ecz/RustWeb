...
query fn rename(tx: Transaction, id: Int, title: String) -> Result<Changed, DbError> mutates Article by id sql {
...
page fn show(ctx: PageContext, id: Int) -> Result<Json, PageError> {
    let article = byId(db, id)?;
    return Ok(json(expose(article, id, title)));
}
route show GET "/articles/:id<Int>" public => show;
"#;
