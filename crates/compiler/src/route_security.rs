...
pub(super) fn parse_explicit_access(
    ...
    match tokens.get(*cursor).map(String::as_str) {
        ...
        _ => Err(CompileError::security(
            ...
            Some(
                "add `public` for intentionally public endpoints, or `auth user`, `auth mfa`, `auth role <Role>`, or `auth permission <Permission>`, `auth permission <Permission> mfa`, or `auth critical <Operation>`"
                    .into(),
            ),
        )),
    }
...
pub(super) fn apply_default_external_string_bounds(
    fields: impl Iterator<Item = FormField>,
    validations: &mut Vec<ValidationRule>,
) {
    for field in fields {
        if field.ty == ValueType::String
            && !validations.iter().any(|rule| {
                rule.field == field.name && matches!(&rule.kind, ValidationKind::Length { .. })
            })
        {
