...
RWLang records security-relevant handler effects in compiler metadata instead of requiring normal application code to repeat effect lists.
...
```rw
page fn article(ctx: PageContext, db: Db, id: ArticleId) -> Result<Json, PageError> {
...
```rw
page fn article(ctx: PageContext, id: ArticleId) -> Result<Json, PageError> {
    let article = byId(db, id)?;
    return Ok(json(expose(article, id, title)));
}
```
