...
RWLang treats file upload as a platform-owned state transition rather than as a trusted filename or MIME string supplied by the client.

Raw files stay private:

```rw
action fn ingest(ctx: ActionContext, file: Upload) -> Result<Json, PageError> {
    return Ok(json(true));
}
