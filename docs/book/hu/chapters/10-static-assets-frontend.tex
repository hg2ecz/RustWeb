...
Az RWLang ezért a statikus asseteket külön, \textbf{read-only static rootból} szolgálja ki. Ez nem ugyanaz a terület, mint az AppFs írható data rootja, és nem ugyanaz a fogalom, mint a felhasználó által feltöltött média.

A fejezet célja négy kérdés tisztázása:

\begin{enumerate}
  \item hol legyen a CSS, kép, font és egyéb build artifact;
  \item hogyan kapjon hosszú cache-élettartamot anélkül, hogy régi verzió ragadna a kliensnél;
  \item mikor szolgálja ki az RWLang, és mikor érdemes Apache/Nginx elé tenni;
  \item mit jelent a V1 CSP-modell a JavaScript szempontjából.
\end{enumerate}
...
Az RWLang fingerprinteltnek tekinti azt a fájlnevet, amelyben legalább 8 hexadecimális karakterből álló, ponttal, kötőjellel vagy underscore-ral határolt token szerepel. Ilyen fájlnál a response hosszú immutable cache-t kaphat:
...
Az RWLang nem akar frontend build rendszert előírni. A pipeline feladata egyszerűen az, hogy determinisztikus outputot készítsen a \code{public/} könyvtárba.
...
Az RWLang server request közben nem tömöríti a statikus fájlt. A release pipeline opcionálisan előre elkészítheti:
...
\section{CSS, kép és font az RWLang HTML-ben}
...
\section{Fontos V1-határ: JavaScript kiszolgálható, de RWLang HTML-ben nem fut}

A static server ismeri a \code{.js} és \code{.mjs} MIME típust, ezért ilyen build artifact fizikailag kiszolgálható. Ez azonban \textbf{nem jelenti azt, hogy az RWLang által renderelt HTML scriptet futtathat}.
...
\begin{enumerate}
  \item külön frontend alkalmazás fut a saját originjén, és RWLang JSON API-t hív;
  \item a kód a browser API kliens viselkedését szemlélteti, nem azt, hogy V1 RWLang HTML-be script tag írható.
\end{enumerate}

Ha az alkalmazás klasszikus RWLang szerver-renderelt oldal, építs HTML + CSS + typed form/action modellre. Ha teljes JavaScript frontend szükséges, kezeld azt külön frontendként, és az RWLang legyen typed JSON backend.

\section{RWLang szolgálja ki vagy Apache/Nginx?}

Két működő deployment modell van.

\subsection*{1. RWLang-owned static namespace}

Ez legyen az alapértelmezett. A reverse proxy minden requestet az RWLang szervernek ad, és az RWLang kezeli az \code{/assets/} namespace-t is:
...
\midrule
szerver-renderelt RWLang oldal & RWLang static root + CSS/image/font \\
kis/közepes production & reverse proxy mindent RWLangnak továbbít \\
nagy statikus forgalom & Apache/Nginx direct static, azonos policy-vel \\
teljes JavaScript frontend & külön frontend deployment + RWLang JSON API \\
