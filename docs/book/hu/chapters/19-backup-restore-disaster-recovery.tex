...
A backup célja nem az, hogy legyen valahol egy másolat, hanem az, hogy egy ismert recovery pontról \textbf{bizonyíthatóan újraindítható legyen az alkalmazás}. Egy RWLang rendszer tartós állapota ráadásul nem feltétlenül egyetlen adatbázis: az application DB, a local-auth identity store és az AppFs együtt is alkothat üzleti állapotot.

Ezért a helyes kérdés nem az, hogy ``mentjük-e az adatbázist?'', hanem az, hogy \textbf{mi a teljes recovery state, milyen konzisztencia mellett mentjük, és rendszeresen bizonyítjuk-e a visszaállítást}.
...
Ha például az RPO 24 óra, egy napi backup elvileg elegendő lehet. Ha az RPO néhány perc, már DB-szintű WAL/binlog/PITR vagy más infrastruktúra kellhet. Az RWLang runtime nem tud szervezeti RPO/RTO-t kitalálni; ezt az operatornak kell a backup technológiára lefordítania.
...
Az RWLang nem próbálja elrejteni a natív adatbázis-backup mechanizmusokat. Az operator a választott backend támogatott eszközével dolgozzon, lehetőleg külön backup credentialdel.
...
pg_restore --no-owner --no-acl \
  --dbname rwlang_restore_test app.dump
\end{lstlisting}
...
\begin{lstlisting}
sqlite3 /srv/rwlang/data/app.db \
...
\begin{lstlisting}
sqlite3 /srv/rwlang/auth/users.db \
...
-> start rwlang-server on isolated listener
