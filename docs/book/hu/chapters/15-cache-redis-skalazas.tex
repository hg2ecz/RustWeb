...
Egyetlen RWLang szerverpéldánynál sok állapot természetesen lokálisnak látszik. Amint viszont két vagy több példány szolgálja ki ugyanazt az alkalmazást, azonnal felmerül három kérdés: hol él a session, ki számolja a rate limitet, és melyik példány cache-e tekinthető érvényesnek? A production válasz ezekre nem az, hogy az alkalmazáskód közvetlenül Redis kulcsokat kezel. Az RWLang a megosztott infrastruktúrát runtime-capabilityként használja, az alkalmazás pedig deklaratív policy-t kér.

Ez a fejezet négy, egymással összefüggő témát választ szét:

\begin{itemize}
  \item \textbf{session és MFA replay state}: felhasználói biztonsági állapot;
  \item \textbf{route rate limit}: forgalomszabályozási állapot;
  \item \textbf{public page cache}: újraszámítható teljesítményállapot;
  \item \textbf{multi-instance deployment}: annak eldöntése, mely állapotnak kell shared backendben élnie.
\end{itemize}

A közös production backend jelenleg Redis, de az alkalmazás nem kap általános Redis API-t. Ez fontos biztonsági és hordozhatósági határ.
...
Productionben az \code{allow_insecure = false} legyen az alap. A Redis hálózati elérhetősége önmagában nem jogosultsági modell: korlátozd hálózati ACL-lel/firewallal is, és a credentialt csak az RWLang service user olvashassa.

A runtime külön namespace-eket használ a különböző célokra. A public cache például \code{rw-cache} namespace-ben él; az alkalmazás nem választ tetszőleges key prefixet.

\section{Shared session több szerverpéldánnyal}

Fejlesztéshez használható in-memory session store, de több application instance esetén ez hibás production modell lenne. Képzelj el két szervert:
...
Ha a session csak A memóriájában élne, akkor egy következő request B-re kerülve a felhasználó autentikációja eltűnne. Sticky sessionnel ezt részben el lehetne fedni, de az failover, rollout és load balancing szempontból törékeny. A helyes megoldás a shared session backend.
...
Cache miss után több azonos request egyszerre érkezhet. Az RWLang ugyanazon instance-en ugyanarra a cache keyre single-flight jelleggel csak egy requestet enged újraépíteni; a többiek várnak, majd újra cache-t olvasnak.

Ez fontos, de a határát is ismerni kell: V1-ben nincs distributed single-flight lease. Két külön RWLang instance ugyanannak a lejárt kulcsnak az újraépítését egyszerre is elkezdheti. Ezért a cache-elhető page queryje önmagában is legyen ésszerű költségű és jól indexelt. A cache nem mentség drága, kontrollálatlan adatbázis-műveletre.

\section{Redis hiba cache-elt request közben}

A public cache backend hibája nem változhat csendben más konzisztenciamodellre. Cache backend elérhetetlenségnél az RWLang \code{cache_unavailable} hibával szolgáltatáshibát adhat a cache-elt útvonalon ahelyett, hogy implicit, korlátlan fallbackként minden requestet az adatbázisra engedne.
...
IP-scope esetén az RWLang nem vakon bízik az \code{X-Forwarded-For} fejlécben. Előbb a trusted proxy policy alapján állapítja meg az effective client IP-t. Emiatt Apache/Nginx mögött a proxy CIDR helyes konfigurálása rate-limit security kérdés is.
...
\begin{itemize}
  ...
  \item \textbf{RWLang instance}: statelesshez közelítő alkalmazásprocessz, lokális bounded runtime state-tel;
...
\begin{itemize}
  \item \code{rw_cache_hits_total} és \code{rw_cache_misses_total};
  \item 429 válaszok és \code{rw_rate_limit_denials_total};
...
\begin{enumerate}
  ...
  \item ezt bármelyik RWLang instance megteheti;
