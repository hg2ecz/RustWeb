...
Production security is partly declared by the RWLang program itself:
...
This is not a second secret/configuration system. It states requirements; the server/operator configuration still supplies certificates, paths and trusted networks. In other words, the RWLang source says \emph{what must be true}, while trusted deployment configuration says \emph{how this environment makes it true}. Startup fails if the real topology cannot satisfy the contract. Source reload cannot silently change it; changing production security requirements requires a restart and explicit operator review.
