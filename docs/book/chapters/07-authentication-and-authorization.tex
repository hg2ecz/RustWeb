...
The limiter is deliberately multi-dimensional rather than a single username+IP counter. The platform simultaneously tracks source+principal, principal across sources, and source across principals; MFA/recovery attempts use a stricter stage. This blocks both distributed guessing against one account and credential stuffing from one source. A successful login clears the concrete pair but does not erase source-wide or principal-wide abuse history. Shared limiter-store failure is fail closed.
