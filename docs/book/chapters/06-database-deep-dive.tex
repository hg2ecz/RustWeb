\chapter{Database deep dive: transactions, migrations, and concurrency}

The previous chapter introduced typed SQL and the basic CRUD pattern. A real wiki, CMS, or line-of-business application soon reaches a point where the question is no longer merely \emph{how to run a SELECT}, but how to preserve data integrity across concurrent requests, multiple server instances, and multiple releases.

This chapter provides that developer model. The goal is to treat the database not as a hidden global service but as an explicit capability, and to treat the schema as a versioned release component.

\section{Three separate responsibilities}

In an RWLang application it is useful to separate three layers:
...
The current V1 data-layer secure defaults include, among other things, a maximum pool size of 32 connections, a 5-second connection-acquire timeout, a 10-second query timeout, a 10,000-row result limit, and a 16 MiB result-size limit. Do not treat these as application logic: correct queries should still be short, paginated, and indexable.

With multiple server instances, the operator must budget total database connections across all instances:

\begin{lstlisting}
total DB connections ~= number of RWLang instances
                       * pool per instance
...
A successful block commits; an error rolls back. \code{Transaction} cannot escape the block, so a closed transaction handle cannot accidentally be used later.
...
The edit form sends the \code{version} value the user saw when opening the page. The \code{Changed} contract is strict: the mutation must affect \textbf{exactly one row}. One changed row is success; zero changed rows become \code{409 Conflict}; two or more rows become a DB error because the query violated its own invariant. \code{Changed} is only valid as the return type of a mutating query, and in V1 it cannot be combined with a \code{RETURNING} clause.
...
RWLang typed SQL protects query shape; it does not replace the database optimizer.
...
RWLang V1 supports all three backends, but that does not make every SQL dialect feature portable.
...
Full application state -- database, AppFs, local-auth store, recovery manifest, RPO/RTO, and restore drills -- is covered in Chapter~\ref{ch:recovery}. It matters here because schema-change planning must already know what state is available for rollback or restoration.
...
\begin{itemize}
\item public read: \code{Article?} by slug;
\item admin list: paginated \code{List<Article>};
