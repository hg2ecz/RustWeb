...
Earlier chapters used server configuration in a task-oriented way. This chapter gives the complete V1 production configuration from an operator's perspective in one place. The reference is based on the actual parser and \code{config/server.toml.sample}; if they ever differ, parser behavior is authoritative.
...
\midrule
\path{server.app} & none & Absolute path to the application's \code{main.rw}. Sample: \code{/srv/rwlang/current/main.rw}. \\
...
Behind Apache or Nginx, RWLang typically does not listen on a public interface:
...
The proxy terminates public TLS. RWLang certificate/key settings are omitted, but \path{tls.public_host} is still required for trusted Host/Origin pinning. The \path{web.trusted_proxy_cidrs} list defines which upstreams may supply authoritative forwarding metadata. In this production mode, a non-TLS RWLang listener is allowed only on loopback.
...
\path{auth.login_max_attempts} & \code{8} & Maximum login attempts in one rate-limit window. \\
\path{auth.login_window_secs} & \code{300} & Login rate-limit window in seconds. \\
\bottomrule
\end{longtable}
...
\midrule
\path{storage.data_root} & none & AppFs runtime-data root. Sample: \code{/srv/rwlang/data}. \\
...
\path{static_assets.root} & none & Read-only static root. Sample: \code{/srv/rwlang/current/public}. \\
...
Key & Default & Meaning \\
\midrule
\path{rate_limit.policies_file} & none & Route rate-limit policy file. Sample: \path{/usr/local/etc/rwlang/rate-limits.toml}. \\
...
When multiple \code{rwlang-server} instances run, shared session/cache/rate-limit state must be carried by Redis. Memory fallback is appropriate only deliberately, usually in a single-instance environment.
...
\path{limits.max_instructions} & \code{100000} & Sample: \code{5000000}. Interpreter budget; increases should be audited. \\
...
\path{limits.resource_profiles_file} & none & Sample: \path{/usr/local/etc/rwlang/resource-profiles.toml}. \\
\bottomrule
\end{longtable}

\subsection*{Why separate request, runtime, and process limits?}

\code{max_body_bytes} limits network input, \code{max_runtime_alloc_bytes} limits interpreter allocations for one request, and \code{max_process_memory_bytes} limits the entire process address space. None replaces the others.
...
Creating and delegating the cgroup directory is an operator task; the server is not a system-administration tool. In the repository's systemd baseline, the service manager is the cgroup authority (\code{MemoryMax}, \code{CPUQuota}, \code{TasksMax}), so the \code{[cgroup]} section is intentionally not enabled. Use RWLang-side cgroup writes only with a separately delegated writable cgroup v2 directory. Do not configure both authorities at once.
...
\begin{enumerate}
  ...
  \item run \code{rwlang-server --check-config};
