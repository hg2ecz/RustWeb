...
A production web application is not operable merely because it ``prints something to the console'' when something goes wrong. Developers and operators need to correlate the same events: which HTTP request ran, which route it reached, what status it returned, whether a security denial occurred, and whether a durable business audit record was created.

RWLang provides four separate surfaces for this:
...
A client-provided \code{X-Request-Id} is not authority. RWLang generates its own request ID so an attacker cannot force the server to use an attacker-chosen correlation identifier.
...
This is a privacy and security invariant. If a business event requires durable information, do not dump the request body; use explicit narrow business-audit fields.

\section{Security audit and HTTP denials}

For trust-boundary denials, RWLang emits a dedicated \code{security_audit} event with the same request ID. This includes invalid/untrusted forwarding metadata returning 400, security decisions returning 401/403/421/426/429, and more precise categories for CSRF/CORS/origin denials. This lets operators distinguish, for example:
...
Access and security logs are operational event streams. Publishing an article, approving an invoice, or changing an order state is business state change and is better recorded transactionally in the application database.
...
RWLang writes to the \code{_rw_business_audit} runtime table fields such as:
...
Security monitoring is now a structured platform boundary, not an ad-hoc log message convention. Automatic events cover authentication abuse, MFA failure, policy/CSRF/origin/CORS denial, webhook verification failure, idempotency conflict and resource exhaustion.
...
\code{rw_policy_denials_total} tracks security-policy denials: in addition to 403/421/426 decisions, a 400 explicitly classified as forwarding-metadata failure counts too. A normal malformed-request 400 does not.
...
RWLang writes file logs in append mode. Let the operating system handle log rotation in production. The repository includes \code{examples/logrotate/rwlang}.
...
File logging uses a bounded writer queue. If the queue fills, a file sink fails, or the writer is unavailable, RWLang falls back to stderr and increments:
...
If the loopback RWLang endpoint is healthy but the public domain is not, inspect Apache/Nginx, TLS, or forwarded-header policy next. If loopback readiness is also 503, investigate backend dependencies first.
...
\begin{itemize}
  ...
  \item increases in \code{rw_runtime_budget_exceeded_total};
  \item increases in \code{rw_log_fallback_total};
...
Retention is an organizational and legal policy. RWLang does not impose one universal retention period.
...
\begin{itemize}
  ...
  \item \code{rw_log_fallback_total} is monitored;
