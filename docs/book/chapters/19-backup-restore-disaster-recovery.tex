...
The purpose of a backup is not merely to keep a copy somewhere. It is to make the application \textbf{provably restartable from a known recovery point}. An RWLang system may also have more than one durable state store: the application database, the local-auth identity store, and AppFs may together form business state.

The right question is therefore not ``do we back up the database?'' but \textbf{what is the complete recovery state, under what consistency boundary is it captured, and do we regularly prove that it can be restored?}
...
If the RPO is 24 hours, a daily backup may be sufficient in principle. If the RPO is measured in minutes, database-level WAL/binlog/PITR or equivalent infrastructure may be required. The RWLang runtime cannot invent organizational RPO/RTO values; operators must translate those requirements into backup technology.
...
RWLang does not attempt to hide native database backup mechanisms. Operators should use the supported tooling of the selected backend, preferably with a dedicated backup credential.
