...
A web application is not correct merely because it compiles. The RWLang compiler intentionally rejects many mistakes before execution, but it cannot prove that a business workflow is correct, that the database is actually reachable, that a migration is compatible, that authorization policy matches the product requirements, or that the production topology works correctly.

Testing in RWLang is therefore best treated as a set of layers that build on one another:
...
\section{The first gate: \texttt{rwlang-cli check}}
...
\begin{description}
  \item[Compile-time error] The source contract is invalid. \code{rwlang-cli check} or the build stops. Fix the code.
...
RWLang closed sum types make security-relevant exceptional states testable at compile time. A \code{match} has no catch-all wildcard, so adding a new enum/sum variant forces every affected match site to be reviewed.
...
Inside the RWLang source tree, the full release-evidence entry point is:

\begin{lstlisting}
./verify.sh
\end{lstlisting}

This is much more than an application source check. It runs Rust workspace tests, positive and negative RWLang fixtures, migration/auth/storage/runtime checks, and release-artifact invariants.

If you modify the RWLang implementation itself, this is the release minimum. If you only develop your own \code{.rw} application, the daily loop can be smaller, but your release process should still include source check, migration/config validation, relevant integration tests, and post-deployment smoke testing.
...
\begin{enumerate}
  \item Does it compile? Run \code{rwlang-cli check}.
...
\begin{itemize}
  \item \code{rwlang-cli check} is green;
...
The goal of testing is not 100 percent line coverage. It is repeatable evidence for the system's important contracts: \textbf{what we allow, what we deny, what state we create, and how we behave when dependencies fail}.
