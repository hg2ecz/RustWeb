...
RWLang therefore serves static assets from a separate, \textbf{read-only static root}. This is not the same area as the writable AppFs data root, and it is not the same concept as user-uploaded media.

This chapter answers four questions:

\begin{enumerate}
  \item where CSS, images, fonts, and other build artifacts should live;
  \item how they can have a long cache lifetime without trapping clients on old versions;
  \item when RWLang should serve them and when Apache/Nginx may serve them directly;
  \item what the V1 CSP model means for JavaScript.
\end{enumerate}
...
RWLang treats a filename as fingerprinted when it contains a token of at least eight hexadecimal characters delimited by a dot, hyphen, or underscore. Such a file may receive a long immutable cache policy:
...
RWLang does not prescribe a frontend build system. The pipeline only needs to produce deterministic output into \code{public/}.
...
The RWLang server does not compress static files during the request. A release pipeline may prebuild:
...
\section{CSS, images, and fonts in RWLang HTML}
...
\section{Important V1 boundary: JavaScript may be served, but not executed in RWLang HTML}

The static server knows the MIME types for \code{.js} and \code{.mjs}, so those build artifacts can physically be served. This does \textbf{not} mean that HTML rendered by RWLang may execute scripts.
...
\begin{enumerate}
  \item a separate frontend application runs on its own origin and calls an RWLang JSON API;
  \item the code illustrates browser-API client behavior, not that a script tag can be embedded in V1 RWLang HTML.
\end{enumerate}

For a classic RWLang server-rendered application, build around HTML + CSS + typed form/action. If a full JavaScript frontend is required, deploy it separately and use RWLang as a typed JSON backend.

\section{Should RWLang serve assets, or Apache/Nginx?}

Two deployment models work.

\subsection*{1. RWLang-owned static namespace}

This should be the default. The reverse proxy sends every request to RWLang, which handles the \code{/assets/} namespace itself:
...
\midrule
server-rendered RWLang page & RWLang static root + CSS/image/font \\
small/medium production & proxy forwards everything to RWLang \\
high static traffic & Apache/Nginx direct static with equivalent policy \\
full JavaScript frontend & separate frontend deployment + RWLang JSON API \\
