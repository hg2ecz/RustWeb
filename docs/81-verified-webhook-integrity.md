...
RWLang webhook endpoints are a distinct request authority. They are not `public` routes and they do not rely on browser sessions or CSRF tokens.
...
Current surface limitation: webhook bodies use RWLang's typed scalar JSON object boundary. Nested provider-specific event schemas should be added as a later typed webhook-schema feature rather than exposing a raw JSON escape hatch.
