...
RWLang treats web security metadata as part of static type checking rather than as optional framework advice.
...
RWLang deliberately does not require application authors to write wrapper-heavy types such as `Authorized<Profile, Read>` in ordinary handler code. `authorize` refines the existing variable instead. This keeps the source ergonomic while preserving a proof in the compiler's static environment.
