...
RWLang treats dependency authority as reviewed release state, not as an incidental Cargo detail.
...
3. `RW-SUPPLY-CHAIN.lock` binds the exact hashes of both files and the accepted provenance/capability model.
...
This is intentionally stricter than Cargo itself. A new dependency cannot silently appear: every direct external dependency edge must have exactly one reviewed capability row. A dependency update cannot silently alter release state: it makes `RW-SUPPLY-CHAIN.lock` stale.
