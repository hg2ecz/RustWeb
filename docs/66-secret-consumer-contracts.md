...
RWLang treats `Secret<T>` as a compile-time data classification, not as a cosmetic alias.
...
```rw
query fn lookupByToken(
    db: Db,
    token: Secret<String>
) -> Result<Credential?, DbError> sql {
...
RWLang follows this rule for future secret-consuming APIs:
