...
check_allowed_deps server "auth compiler data integrations language-core runtime storage observability"
...
grep -q '^pub use effect::Effect;$' crates/language-core/src/lib.rs \
  || fail 'language-core facade must re-export Effect'
grep -q 'execute_request_with_profiles_and_outbound' crates/runtime/src/lib.rs \
  || fail 'runtime facade must re-export outbound-aware request execution'
...
test -f RW-SUPPLY-CHAIN.lock || fail 'missing reviewed supply-chain lock envelope'
grep -q '^provenance crates.io-checksummed-only$' RW-SUPPLY-CHAIN.lock \
  || fail 'supply-chain provenance must remain checksummed crates.io only'
grep -q '^capability-model direct-external-dependency-edge$' RW-SUPPLY-CHAIN.lock \
...
grep -q 'MAX_RWLANG_STATUS_BODY_BYTES: usize = 256 \* 1024' crates/integrations/src/https_client.rs \
  || fail 'RWLang outbound status calls must retain a hard response-body cap'
...
grep -q 'request_external_bytes: u64' crates/runtime/src/execution_context.rs \
  || fail 'runtime request budget must retain cumulative external I/O accounting'
