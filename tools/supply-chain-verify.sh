...
TMP=$(mktemp -d "${TMPDIR:-/tmp}/rwlang-supply.XXXXXX")
...
if [ "$MODE" != "--no-envelope" ]; then
  [ -f RW-SUPPLY-CHAIN.lock ] || { echo 'supply-chain verification refused: RW-SUPPLY-CHAIN.lock is missing' >&2; exit 1; }
  cargo_hash=$(sha256sum Cargo.lock | awk '{print $1}')
  policy_hash=$(sha256sum "$POLICY" | awk '{print $1}')
  grep -Fxq 'format rwlang-supply-chain-v1' RW-SUPPLY-CHAIN.lock || { echo 'invalid supply-chain lock format' >&2; exit 1; }
  grep -Fxq "cargo-lock-sha256 $cargo_hash" RW-SUPPLY-CHAIN.lock || { echo 'RW-SUPPLY-CHAIN.lock is stale for Cargo.lock' >&2; exit 1; }
  grep -Fxq "capability-policy-sha256 $policy_hash" RW-SUPPLY-CHAIN.lock || { echo 'RW-SUPPLY-CHAIN.lock is stale for capability policy' >&2; exit 1; }
  grep -Fxq 'provenance crates.io-checksummed-only' RW-SUPPLY-CHAIN.lock || { echo 'unexpected supply-chain provenance policy' >&2; exit 1; }
  grep -Fxq 'capability-model direct-external-dependency-edge' RW-SUPPLY-CHAIN.lock || { echo 'unexpected supply-chain capability model' >&2; exit 1; }
fi
